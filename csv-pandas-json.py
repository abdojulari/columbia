...
def checkup(x):
    if len(x) == 4:
        ...
        return 'D'
df['specificity'] = df['DateAr'].map(checkup)
df.DateAr = pd.to_datetime(df.DateAr)  
mask = df['DateDpt'].str.len() == 4 
df['specificity'] = df['DateDpt'].map(checkup)
#print mask
df.DateDpt = pd.to_datetime(df.DateDpt)
df.loc[mask, 'DateDpt' ] +=  pd.offsets.YearEnd()
...
for i, data in df.iterrows():
    ...
    df2 = df2[['AuthorID', 'ArCiCo', 'DptCiCo', 'DateAr', 'DateDpt', 'specificity']]
...
for arrival_date, data in df2.groupby('DateAr'):
	...
	#print flights.DateAr
        dup_date = flights.drop(['DateAr', 'DateDpt', 'ArCiCo', 'DptCiCo'], axis=1)
        json_dict[arrival_date.strftime('%Y-%m-%d')][city] = dup_date.to_dict(orient="records")#[str(v) for v in flights.AuthorID]
